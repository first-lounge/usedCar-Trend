...
- 대용량 데이터를 저장하는 경우라면?
  > execut 방식이 대용량 데이터를 저장할 때도 효율적인지에 대한 의문이 생겨 테스트를 진행하였습니다.
...
- 성능 차이 발생 이유
  - execute 같은 경우 매번 commit을 호출하고 to_sql은 모든 데이터를 한 번에 넣기 떄문에 commit이 한 번만 호출됩니다.
  - commit을 많이 호출하게 되면 디스크 i/o가 많이 발생하여 성능을 저하시킬 가능성이 높습니다.
